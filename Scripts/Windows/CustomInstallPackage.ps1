# --------------------------------------------------------------------------------------------------------------------
# <copyright file="CustomInstallPackage.ps1" company="VMware, Inc.">
...
function IsWs1Installed()
{
    [bool] $retVal = $true
    $item = (Get-ChildItem "Registry::HKEY_CLASSES_ROOT\ActivatableClasses\Package" -Recurse | where-object {($_.Name -like "*AirWatchLLC.VMwareWorkspaceONE*")})
    if($item -eq $null){
         $retVal = $false
         Log "Error" "Could not find WS1 application in the registry path."
    }
    Write-Output $retVal
}
    ...
    if($continue -eq $true)
    {
        ...
        $IsWorkspaceOneInstalled = $false

	    #Appx Bundle filename should match with uploaded WorkSpace application.
	    $appxBundlepath = Join-Path -Path $appxPath -ChildPath "668f4ce67ec547f3a39a59e031b8d07b.appxbundle";
	
	    Log "INFO" ("Installing: " + $appxBundlepath);
	    ...
	    try 
	    {
			...
			Do
			{
				switch ( $dependenciesArchitecture )
				{
					"x86"
						...
						Add-AppxPackage -Path $appxBundlepath -ForceApplicationShutdown -DependencyPath $dep1,$dep2,$dep3
						}
					"x64"
						{
					...
					"ARM"
						...
						Add-AppxPackage -Path $appxBundlepath -ForceApplicationShutdown -DependencyPath $dep1,$dep2,$dep3,$dep4
						}
					default
						{
		    ...
		    #Gets last command Execution status.
		    if(! $?)
		    {
			    Log "ERROR" "Installation of WorkspaceOne failed. Please check event logs for more details."
		    }
						
			if($IsWorkspaceOneInstalled -eq $true)
			{
				Log "INFO" "Installation of WorkspaceOne Complete."
			}
